...
if [ "$GENESIS_FILE" = "" ]; then
  DEFAULT_GENESIS_FILE_PATH="${HOME}/.rollapp-wasm/config/genesis.json"
...
update_params() {
  ...
  if ! jq '.app_state.gasless.params.minimum_gas_deposit[0].amount = "10000000000000000000"' "$TEMP_GENESIS" > "${TEMP_DIR}/temp.json"; then
  ...
  # Update jq command to use temp file
  if ! jq '.app_state.tokenfactory.params.denom_creation_fee[0].amount = "10000000000000000000"' "$TEMP_GENESIS" > "${TEMP_DIR}/temp.json"; then
    echo "Error updating JSON file"
    success=false
  else
    mv "${TEMP_DIR}/temp.json" "$TEMP_GENESIS"
