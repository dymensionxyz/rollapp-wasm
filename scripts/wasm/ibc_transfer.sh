...
# get ics20 wasm port
wasm_port=$(rollappd q wasm contract-state smart $ICS20_ADDR '{"port":{}}' | grep "port_id:" | cut -d' ' -f4)
...
channel_id=$(rollappd q wasm contract-state smart $ICS20_ADDR '{"list_channels":{}}' | grep "channel_id:" | tail -n 1 | awk '{print $2}')
